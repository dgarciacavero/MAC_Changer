...
def GetMac(interface):
    ...
    return re.search("\w\w:\w\w:\w\w:\w\w:\w\w:\w\w", ifconfig_result).group(0)

def CheckChange(interface, new_value):
    actual_value = GetMac(interface)
    if actual_value == new_value:
        print("\n[+] " + interface + "'s mac address succesfully changed to " + new_value)
    else:
